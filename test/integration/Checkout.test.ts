...
test("Deve fazer o pedido com cupom de desconto expirado", async function() {
    const repositoryFactory = new RepositoryMemoryFactory();
    const itemRepository = repositoryFactory.createItemRepository();
    const orderRepository = repositoryFactory.createOrderRepository();
    const couponRepository = repositoryFactory.createCouponRepository();
...
test("Deve fazer o pedido com cupom de desconto não expirado", async function() {
    const repositoryFactory = new RepositoryMemoryFactory();
    const itemRepository = repositoryFactory.createItemRepository();
    const orderRepository = repositoryFactory.createOrderRepository();
    const couponRepository = repositoryFactory.createCouponRepository();
    ...
    const input = {
        ...
        coupon: "VALE20",
        date: new Date("2020-03-01T10:00:00")
    }
    await checkout.execute(input);
    const getOrdersByCpf = new GetOrdersByCpf(orderRepository);
    const orders = await getOrdersByCpf.execute("317.153.361-86")
